    ...
    def train_model(self):
        ...
        for epoch in range(self.epochs):
            ...
            if f1_score_micro>best_score:
                torch.save(self.model,os.path.join(self.output_dir, self.model_path[-10:] + "_" + self.target + '_cross' +  "_model.pt"))
...
train_dataframe = pd.read_csv('preprocessing/train_cleaned_new.csv') # Load pre-processed train set
valid_dataframe = pd.read_csv('preprocessing/valid_cleaned_new.csv') # Load pre-processed validation set
train_embedding = np.load('preprocessing/train_emoji.npy') # Load train emoji embeddings
valid_embedding = np.load('preprocessing/valid_emoji.npy') # Load validation emoji embeddings

# Train model using Indic-BERT
classifier = binary_classifier(train_dataframe,valid_dataframe,"ai4bharat/indic-bert",'non-hostile',10,1e-5,train_embedding,valid_embedding)
classifier.train_model()

# Train model using mBERT
classifier = binary_classifier(train_dataframe,valid_dataframe,"bert-base-multilingual-uncased",'non-hostile',10,1e-5,train_embedding,valid_embedding)
classifier.train_model()
